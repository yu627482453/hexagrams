...
public class ImageUtils {
	...
	static {
		...
		RESOLVER_LIST.add(new ImageResolver() {
			...
			public void resolve(ImageInfo info, InputStream stream) throws IOException {
				stream.skip(3);
				info.setWidth(StreamUtil.readInt(stream, 2, false));
				info.setHeight(StreamUtil.readInt(stream, 2, false));
		...
		RESOLVER_LIST.add(new ImageResolver() {
			...
			public void resolve(ImageInfo info, InputStream stream) throws IOException {
				...
				do {
					...
					int len = StreamUtil.readInt(stream, 2, true);
					if (marker == 192 || marker == 193 || marker == 194) {
						stream.skip(1);
						info.setHeight(StreamUtil.readInt(stream, 2, true));
						info.setWidth(StreamUtil.readInt(stream, 2, true));
	...
	public ImageInfo quickResolveClone(InputStream stream) throws IOException {
		final InputStream[] streams = StreamUtil.clone(stream, 2);
	...
	public ImageInfo mixResolveClone(InputStream stream) throws IOException {
		InputStream[] streams = StreamUtil.clone(stream, 2);
	...
	private void tiffResolver(InputStream stream, ImageInfo info, int r1) throws IOException {
		...
		int ifd = StreamUtil.readInt(stream, 4, bigEndian);
		int entries;
		stream.skip(ifd - 8);
		entries = StreamUtil.readInt(stream, 2, bigEndian);
		for (int i = 1; i <= entries; i++) {
			int tag = StreamUtil.readInt(stream, 2, bigEndian);
			int fieldType = StreamUtil.readInt(stream, 2, bigEndian);
			int valOffset;
			if ((fieldType == 3 || fieldType == 8)) {
				valOffset = StreamUtil.readInt(stream, 2, bigEndian);
				stream.skip(2);
			}
			else {
				valOffset = StreamUtil.readInt(stream, 4, bigEndian);
			}
			if (tag == 256) {
