package cn.bipher.hexagrams.redis.template;
...
public class RedisRepository {
     ...
     */
    public long dbSize() {
        return Optional.ofNullable(redisTemplate.execute(RedisServerCommands::dbSize)).orElse(0L);
    }
    ...
    public boolean del(final String... keys) {
        ...
        for (String key : keys) {
            result = Optional.ofNullable(redisTemplate.delete(key)).orElse(false);
        }
        return result;
    }
    ...
    public List getList(String key, int start, int end, RedisSerializer valueSerializer) {
        byte[] rawKey = rawKey(key);
        return redisTemplate.execute(connection -> deserializeValues(connection.lRange(rawKey, start, end),
